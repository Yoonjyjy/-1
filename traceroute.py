...
class ipheader:
    ...
    def assemble(self):
        self.raw = struct.pack('!BBHHHBBH4s4s', self.version, self.tos, self.totalLength, self.id, self.flag, self.ttl, self.prototype, self.chksum, self.saddr, self.daddr)
...
class CICMPheader:
    ...
    def createICMPHeader(self):
        self.raw = struct.pack(
            '!BBHHH', self.icmpType, self.icmpCode, self.icmpChksum, self.icmpId, self.icmpSeq)
    ...
    def makeChksum(self, data):
        ...
        while count < count_to:
            if(data[count + 1] == 32 and count == len(data) - 2):
                thisvalue = data[count]
            else:
                thisvalue = data[count + 1] * 256 + data[count]
...
class CUDPheader:
    ...
    def createUDPHeader(self):
        ...
        Pseudo = struct.pack('!4s4sBBHHHHH', oDict['src'], oDict['dst'], 0x00, 0x11, 8 + len(
            self.data), self.src, self.dst, 8 + len(self.data), self.udpChksum)

        self.udpChksum = self.makeChksum(Pseudo + self.data.encode())

        self.raw = struct.pack('!HHHH', self.src, self.dst,
                               self.length, self.udpChksum)
...
def run(dst, size, t, c, pt, port=33435):
    ...
    for i in range(1, c+1):
        ...
        for j in range(0, 3):
            ...
            if data != None:
                dt = struct.unpack('!4B', data[12:16])
                ds = str(str(dt[0])+'.'+str(dt[1]) +
                         '.'+str(dt[2])+'.'+str(dt[3]))
        if ds != '':
...
def sender(dst, size, ridt, sqnum, c, pt, port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW) as sock:
        ...
        iph = ipheader(dst, idt=ridt, src=adr)

        if pt == 'U':
            iph.prototype = 17
            uh = CUDPheader(size=size, iph=iph, dst=port,
                            src=random.randint(34000, 59000))
            iph.ttl = c
...
def receiver(idt, myaddr, t, pt, port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as sock:
        try:
            ...
            dt = struct.unpack('!4B', data[16:20])
            adr = str(str(dt[0])+'.'+str(dt[1])+'.'+str(dt[2])+'.'+str(dt[3]))
            ...
            if myaddr != adr:
                ...
                if pt == 'I':
                    if data[20] == 11:
                        if data[52]*256 + data[53] != idt:
                            print('       *', end='\t')
                            return None
                    if data[20] == 0:
                        if data[24]*256 + data[25] != idt:
                            print('       *', end='\t')
                            return None
                if pt == 'U':
                    if data[20] == 11:
                        if data[32]*256 + data[33] != idt:
                            print('       *', end='\t')
                            return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', help='RECV_TIMEOUT',
                        required=False, default=0.5, type=float)
    parser.add_argument('-c', help='MAX_HOPS',
